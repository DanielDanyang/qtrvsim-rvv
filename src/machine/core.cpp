...
ExecuteState Core::execute(const DecodeInterstage &dt) {
    ...
    RegisterValue alu_val; // remove out of scope

    switch (dt.aluop.alu_op) {
    case AluOp::VADD_VV: {
    ...
    case AluOp::VADD_VI: {
        uint8_t vd = dt.num_rd;
        uint8_t vs = dt.num_rs;
        int32_t imm = dt.immediate_val.as_i32();

        if (vd >= vector_registers.size() || vs >= vector_registers.size()) {
